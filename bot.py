...
WATERMARK = "NexusNodes VPS"
WELCOME_MESSAGE = "NexusNodes! Get Started With Us!"
...
async def setup_container(container_id, status_msg, memory, vps_id=None, use_custom_image=False):
    ...
    try:
        ...
        # Set NexusNodes customization
        if isinstance(status_msg, discord.Interaction):
            await status_msg.followup.send("🎨 Setting up NexusNodes customization...", ephemeral=True)
        else
            await status_msg.edit(content="🎨 Setting up NexusNodes customization...")
...
@app_commands.describe(
    ...
    use_custom_image="Use custom NexusNodes image (recommended)"
)
async def create_vps_command(ctx, memory: int, cpu: int, disk: int, owner: discord.Member, 
    ...
    try:
        ...
        if use_custom_image:
            ...
            try:
                container = bot.docker_client.containers.run(
                    ...
                    ports={'22/tcp': str(external_port)},
                    volumes={
                        f'nexus-{vps_id}': {'bind': '/data', 'mode': 'rw'}
                    },
                    restart_policy={"Name": "always"}
                )
        ...
        await status_msg.edit(content="🔧 Container created. Setting up NexusNodes environment...")
        await asyncio.sleep(5)
        ...
        try:
            embed = discord.Embed(title="🎉 NexusNodes VPS Creation Successful", color=discord.Color.green())
            ...
            await status_msg.edit(content=f"✅ NexusNodes VPS creation successful! VPS has been created for {owner.mention}. Check your DMs for connection details.")
...
async def delete_vps(ctx, vps_id: str):
    ...
    try:
        ...
        await ctx.send(f"✅ NexusNodes VPS {vps_id} has been deleted successfully!")
    except Exception as e:
...
async def connect_vps(ctx, token: str):
    ...
    try:
        ...
        embed = discord.Embed(title="NexusNodes VPS Connection Details", color=discord.Color.blue())
...
4. You will be connected to your NexusNodes VPS
...
async def admin_stats(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="NexusNodes System Statistics", color=discord.Color.blue())
...
async def vps_usage(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="Your NexusNodes VPS Usage", color=discord.Color.blue())
...
async def global_stats(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="NexusNodes Global Usage Statistics", color=discord.Color.blue())
...
async def reinstall_bot(ctx):
    ...
    try:
        await ctx.send("🔄 Reinstalling NexusNodes bot... This may take a few minutes.")
...
class VPSManagementView(ui.View):
    ...
    async def handle_missing_container(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title=f"NexusNodes VPS Management - {self.vps_id}", color=discord.Color.red())
    ...
    async def start_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            embed = discord.Embed(title=f"NexusNodes VPS Management - {self.vps_id}", color=discord.Color.green())
            ...
            await interaction.followup.send("✅ NexusNodes VPS started successfully!", ephemeral=True)
        except Exception as e:
    ...
    async def stop_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            embed = discord.Embed(title=f"NexusNodes VPS Management - {self.vps_id}", color=discord.Color.orange())
            ...
            await interaction.message.edit(embed=embed)
            await interaction.followup.send("✅ NexusNodes VPS stopped successfully!", ephemeral=True)
        except Exception as e:
    ...
    async def restart_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            await interaction.followup.send("✅ NexusNodes VPS restarted successfully! New SSH details sent to owner.", ephemeral=True)
        except Exception as e:
...
class OSSelectionView(ui.View):
    ...
    async def reinstall_os(self, interaction: discord.Interaction, image: str):
        try:
            ...
            status_msg = await interaction.followup.send("🔄 Reinstalling NexusNodes VPS... This may take a few minutes.", ephemeral=True)
            ...
            try:
                container = bot.docker_client.containers.run(
                    ...
                    ports={'22/tcp': str(vps['external_ssh_port'])},
                    volumes={
                        f'nexus-{self.vps_id}': {'bind': '/data', 'mode': 'rw'}
                    }
                )
            except docker.errors.ImageNotFound:
            ...
            await status_msg.edit(content="✅ NexusNodes VPS reinstalled successfully!")
            ...
            try:
                ...
                try:
                    ...
                    await channel.send(f"❌ Error reinstalling NexusNodes VPS {self.vps_id}: {str(e)}")
                except:
...
class TransferVPSModal(ui.Modal, title='Transfer VPS'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message(f"✅ NexusNodes VPS {self.vps_id} has been transferred from {old_owner_name} to {new_owner_name}!", ephemeral=True)
            
            try:
                embed = discord.Embed(title="NexusNodes VPS Transferred to You", color=discord.Color.green())
...
async def transfer_vps_command(ctx, vps_id: str, new_owner: discord.Member):
    ...
    try:
        ...
        await ctx.send(f"✅ NexusNodes VPS {vps_id} has been transferred from {ctx.author.name} to {new_owner.name}!")
